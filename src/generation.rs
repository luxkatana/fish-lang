        ...
        for node in &self.nodes {
            match node.nodetype() {
                NodeType::Exit => {
                    ...
                    let node_exit_type_param1 = &node
                        .as_any()
                        .downcast_ref::<NodeExit>()
                        .unwrap()
                        .expr;
                    match node_exit_type_param1 {
                        ...
                        ExpressionTypes::Identifier(ident_node) => {
                            ...
                            if let Some(var_stack) = stack_size_of_var {
                                generated_assembly += format!("    push QWORD [rsp + {}]\n", (self.stack_size - var_stack - 1) * 8).as_str();
                                generated_assembly += "    mov rax, 60\n";
                                ...
                                return Err(format!("ERROR: {identifier_name} identifier does not exist"));
                            }
                    ...
                    match var_node.expr.nodetype() {
                        ...
                        NodeType::ExpressionIdent => {
                            let expr = var_node
                                                        ...
                                                        .as_ref()
                                                        .unwrap();
                            let varident = var_node
                                                .identifier
                                                .value
                                                .as_ref()
                                                .unwrap();
                            if let Some(pointed_assignment) = self.variables.get(expr) {
                                generated_assembly += format!("    PUSH QWORD [rsp + {}]\n", (self.stack_size - pointed_assignment - 1) * 8).as_str();
                                generated_assembly += "    pop rax\n";
                                generated_assembly += format!("    push rax; {} = {}\n", varident, expr).as_str();
